...
> Launch command (Alternative: `left-click` the green grasshopper button in the toolbar)
...
> 1. **Main menu bar**: The menu is similar to typical Windows menus. Each menu
>    shows more commands to edit the settings or the canvas itself as well as
...
> 4. The canvas: The canvas is the main workplace where all the components will
